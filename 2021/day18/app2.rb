...
def solve(lines)
  ...
  lines.each do |linex|
    lines.each do |liney|
      ...
      gx = parse_node(JSON.parse(linex))
      gy = parse_node(JSON.parse(liney))

      magnitude = add!(gx, gy).magnitude
...
assert_equal(3993, solve_from_file('test1.txt'), 'Test result failed')
